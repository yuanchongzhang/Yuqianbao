...
public class BonusRecordAdapter extends RecyclerView.Adapter<BonusRecordAdapter.ViewHolder> {
    ...
    private List<JsonPromotionlog.Info.ListContent> list;

    public BonusRecordAdapter(Context context, List<JsonPromotionlog.Info.ListContent> list) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = BoundViewHelper.boundView(this, MyApplication.scaleScreenHelper.loadView((ViewGroup) LayoutInflater.from(parent.getContext())
                .inflate(R.layout.list_04_item_view, null)));
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        holder.tv02.setText(list.get(position).add_time);
        holder.tv03.setText(list.get(position).affect_money);
        holder.tv04.setText(list.get(position).type);
        if (position % 2 == 1) {
