 ...
 */
public class MoneyManagement02Adapter extends RecyclerView.Adapter<MoneyManagement02Adapter.ViewHolder> {

    private Context context;
    private List<JsonProductout.Info.ListContent> list;

    public MoneyManagement02Adapter(Context context, List<JsonProductout.Info.ListContent> list) {
...
// you provide access to all the views for a data item in a view holder
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = BoundViewHelper.boundView(this, MyApplication.scaleScreenHelper.loadView((ViewGroup) LayoutInflater.from(parent.getContext())
                .inflate(R.layout.list_11_item_view, null)));
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        holder.tv01.setText(list.get(position).id);
        holder.tv02.setText("标号: "+list.get(position).borrow_id);
        holder.tv03.setText("借入人："+list.get(position).borrow_user);
        holder.tv04.setText(list.get(position).borrow_interest_rate+"%");
        holder.tv05.setText(list.get(position).investor_capital+"元");
        holder.tv06.setText(list.get(position).investor_interest+"元");
    }
