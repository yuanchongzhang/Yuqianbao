 ...
 */
public class ViewPagerListAdapter extends RecyclerView.Adapter<ViewPagerListAdapter.ViewHolder> {

    private Context context;
    private List<Object> list;
    private int mark;

    public ViewPagerListAdapter(Context context, List<Object> list, int mark) {
...
// you provide access to all the views for a data item in a view holder
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = BoundViewHelper.boundView(this, MyApplication.scaleScreenHelper.loadView((ViewGroup) LayoutInflater.from(parent.getContext())
                .inflate(R.layout.list_14_item_view, null)));
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        if (mark == 0) {
           ...
           // holder.tv02.setVisibility(View.GONE);
            String name = ((JsonInvestdetail.Info.PayingListContent)list.get(position)).borrow_name;
          holder.tv02.setText(name.substring(0,1)+"***"+name.substring(name.length()-1,name.length()));
            holder.tv03.setText(((JsonInvestdetail.Info.PayingListContent)list.get(position)).capital);
            holder.tv04.setText(((JsonInvestdetail.Info.PayingListContent)list.get(position)).interest);
            holder.tv05.setText(((JsonInvestdetail.Info.PayingListContent)list.get(position)).status);
            holder.tv06.setText(((JsonInvestdetail.Info.PayingListContent)list.get(position)).deadline_cn);
