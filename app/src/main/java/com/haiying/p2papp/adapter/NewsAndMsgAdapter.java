...
import com.haiying.p2papp.activity.MsgDetailActivity;
 ...
 */
public class NewsAndMsgAdapter extends RecyclerView.Adapter<NewsAndMsgAdapter.ViewHolder> {


    private Context context;
    private int flag = 0;
    private List<JsonMsglist.Info.ListContent> list;

    public NewsAndMsgAdapter(Context context, int flag, List<JsonMsglist.Info.ListContent> list) {
...
// you provide access to all the views for a data item in a view holder
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = BoundViewHelper.boundView(this, MyApplication.scaleScreenHelper.loadView((ViewGroup) LayoutInflater.from(parent.getContext())
                .inflate(R.layout.list_15_item_view, null)));
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (flag == 0) {
            holder.tv01.setText("收件时间：" + list.get(position).send_time);
            holder.tv02.setText(list.get(position).title);
        } else if (flag == 1) {
            holder.iv01.setVisibility(View.GONE);
            holder.tv01.setText(list.get(position).send_time);
            holder.tv02.setText(list.get(position).title);
        }
...
//                    intent.putExtra("url","https://www.haiying365.com/api/index/article_content?id="+list.get(position).id+"&show_type=article");

                    intent.putExtra("url","http://vhost119.zihaistar.com/api/index/article_content?id="+list.get(position).id+"&show_type=article");
